function clot_generator_3D(startIter, endIter)
...
for index = startIter:endIter
    ...
    numLloydIterations =5;
    % Apply Lloyd's relaxation to the second Voronoi network
    DT_inside = delaunayTriangulation(Points); % Initial triangulation
    for iter = 1:numLloydIterations
        [V, R] = voronoiDiagram(DT_inside);

        % Calculate centroids for each Voronoi cell
        newPoints = zeros(size(Points, 1), 3);
        for i = 1:length(R)
            region = R{i};
            vertices = V(region, :);
            vertices = vertices(~isinf(vertices(:, 1)), :); % Remove infinite vertices
            if ~isempty(vertices)
                newPoints(i, :) = mean(vertices, 1); % Centroid calculation
            else
                newPoints(i, :) = Points(i, :); % If no finite region, keep original
            end
        end

        % Update points for next iteration
        DT_inside = delaunayTriangulation(newPoints);
    end

    % Update Points after Lloyd's relaxation
    Points = newPoints;
    ...
    %             num_RBC_in_region = round((10+10*rand()) * RBC_filling_factor * distances_sorted(1) / RBC_diameter);
