for index = 20:30
    ...
    mesh_size_x = round(max(coordinates(:,1))-min(coordinates(:,1)));
    mesh_size_y = round(max(coordinates(:,2))-min(coordinates(:,2)));
    mesh_size_z = round(max(coordinates(:,3))-min(coordinates(:,3)));
    ...
    scaling_factor_x = mesh_size_x / max(coordinates(:,1));
    scaling_factor_y = mesh_size_y / max(coordinates(:,2));
    scaling_factor_z = mesh_size_z / max(coordinates(:,3));
    coordinates_scaled(:,1) = coordinates(:,1) * scaling_factor_x;
    coordinates_scaled(:,2) = coordinates(:,2) * scaling_factor_y;
    coordinates_scaled(:,3) = coordinates(:,3) * scaling_factor_z;
