%The script to use verasonics with the vibrant laser (without laser control
%system 
% this has both ultrasound and PA capability for comparison 
% version 1
...
flash2Qdelay = 212; % microseconds between trigger input and start of acquisition (which outputs a trigger pulse at time=0)
...
%     Event(n-1).seqControl = [3,9];   % replace last 2D acquisition Event's seqControl (longer TTNA and new RCV profile)
    ...
    for j = (na+1):(na+ne)
        ...
        Event(n).info = 'Send Trigger out at the start of every PA aqcuisition';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 0;
...
%         n = n+1;
    ...
    Event(n-1).seqControl = 4; % replace last PA acquisition Event's seqControl with longer TTNA and RCV profile change
